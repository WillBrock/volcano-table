(function() {
	 ...
	 * @return Collection
	 */
	var normalizeRecords = function() {
		var table_data = this.data.settings.table_data || false;
		...
		// Check if an array or cursor was passed in
			...
			return this.data.settings.collection;
		}
	};
	...
	var initalize = function() {
		...
		this.data.settings.table_data = normalizeRecords.call(this);
	};
		...
		if(tmp_attributes) {
			...
			_.each(tmp_attributes, function(value, key) {
				...
				if(add_record && key === 'class') {
					tmp += ' volcanotable-ui-input volcanotable-add-input volcanotable-add-input-min-width';
				}
			...
			attributes.push('class="volcanotable-ui-input volcanotable-add-input volcanotable-add-input-min-width"');
		}
	...
	Template.volcanoTable.helpers({
		...
		records : function() {
			// @todo figure out why records is called multiple times cause of the reactive vars
			...
			// There may be a beter way of doing this
			...
			var count_record = VolcanoTableCounts.findOne({ _id : passed_template });

			if(count_record) {
				var record_count = count_record.count;
				this.totalPages.set(Math.ceil(record_count / this.paginationNumber));
			}

			// `where` can also be difined on the client
			// Make sure to always to sorting on the client
			return this.settings.collection.find(where, { sort : sort });
		},
	...
	Template.volcanoTable.events({
		...
		'keyup .volcanotable-filter' : function(e) {
			...
			// Return the current page to the first page
		...
		'click .volcanotable table thead th' : function(e, template) {
			...
			// Return the current page to the first page
			template.data.currentPage.set(1);
